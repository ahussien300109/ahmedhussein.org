...
const COURSES = [
  ...
  },
  {
    ...
    objectives: [
      ...
      "Deploy advanced routing protocols",
      "Configure SD-WAN solutions",
      "Automate with Python and Ansible",
  ...
  },
  {
    ...
    objectives: [
      ...
      "Deploy Firepower NGFW",
      "Implement site-to-site and remote VPNs",
  ...
  },
  {
    ...
    objectives: [
      "Navigate Packet Tracer confidently",
      "Build LAN and WAN topologies",
      "Configure routing and switching",
      "Troubleshoot real network issues",
      "Create complex multi-site scenarios",
    ],
    curriculum: [
  ...
  },
  {
    ...
    objectives: [
      "Deploy Cisco SD-WAN fabric",
      "Write Python scripts for network automation",
      "Use Ansible for device configuration",
      "Consume NETCONF/RESTCONF APIs",
      "Automate with DNA Center",
    ],
    curriculum: [
  ...
  },
  {
    ...
    reviews: "156",
    prereqs:
      "Have completed CCNA training or have equivalent networking knowledge.",
    objectives: [
      "Master all 6 CCNA exam domains",
      "Score 90%+ on practice tests",
      "Develop time management strategies",
      "Identify and fix knowledge gaps",
      "Book and pass CCNA exam",
    ],
    curriculum: [
...
const App = {
  ...
  renderAllCourses(filter = "all", query = "") {
    ...
    if (count)
      count.innerHTML = `<span>${filtered.length}</span> courses found`;
  ...
  refreshCourseGrids() {
    ...
    if (fg)
      fg.innerHTML = COURSES.slice(0, 3)
        .map((c) => this.courseCardHTML(c))
        .join("");
  ...
  selectTier(tier) {
    ...
    document
      ...
      .forEach((o) => o.classList.toggle("selected", o.dataset.tier === tier));
  },
  ...
  updateNavForAuth(loggedIn) {
    document.getElementById("nav-signin").style.display = loggedIn
      ? "none"
      : "";
    document.getElementById("nav-register").style.display = loggedIn
      ? "none"
      : "";
    document.getElementById("nav-dashboard-link").style.display = loggedIn
      ? ""
      : "none";
    document.getElementById("nav-logout-btn").style.display = loggedIn
      ? ""
      : "none";
  },
  ...
  showDashPanel(panel) {
    ...
    document
      .querySelector(`.dash-nav-item[data-panel="${panel}"]`)
  ...
  updateDashboard() {
    ...
    document.getElementById("dash-username").textContent =
      this.currentUser.fname || "Student";
    document.getElementById("dash-plan-name").textContent =
      this.currentUser.tier === "premium" ? "Premium ⭐" : "Free Plan";
    document.getElementById("dash-enrolled-count").textContent =
  ...
  saveProfile() {
    ...
    this.currentUser.fname =
      document.getElementById("profile-fname")?.value || this.currentUser.fname;
    this.currentUser.lname =
      document.getElementById("profile-lname")?.value || this.currentUser.lname;
  ...
  sendContact() {
    ...
    ["contact-fname", "contact-lname", "contact-email", "contact-msg"].forEach(
      (id) => {
        const el = document.getElementById(id);
        if (el) el.value = "";
      },
